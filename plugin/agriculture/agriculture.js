...
function render(c){if(!c)return;const list=[...data.activities].sort((a,b)=>(date(a.harvestStart)?.getTime()??Infinity)-(date(b.harvestStart)?.getTime()??Infinity));c.innerHTML=`<section class="edag-page"><div class="edag-intro"><div class="edag-title"><span>🚁</span><strong>Période favorable à la prospection thermique</strong></div><div class="edag-subtitle">Environ 1 mois avant le début de la récolte ou de la fauche.</div><div class="edag-location">📍 ${esc(data.sector)}</div></div><div class="edag-list">${list.map((a,i)=>{const st=currentState(a), raw=String(a.type||'🌾 Culture'), icon=(raw.match(/^\S+/)||['🌾'])[0],name=raw.replace(/^\S+\s*/,'');return `<article class="edag-card ${st}" data-card><div class="edag-row" role="button" tabindex="0" aria-expanded="false"><div class="edag-crop"><span class="edag-icon">${esc(icon)}</span><div><strong>${esc(name)}</strong><small>${fmt(a.harvestStart)} → ${fmt(a.harvestEnd)}</small></div></div><div class="edag-timeline">${timeline(a)}</div><div class="edag-status-wrap">${status(st)}</div><span class="edag-chevron">⌄</span></div><div class="edag-details" hidden><div class="edag-detail-grid"><div><small>📅 Récolte / fauche</small><strong>${fmt(a.harvestStart)} → ${fmt(a.harvestEnd)}</strong></div><div><small>🚁 Prospection thermique</small><strong>${fmt(addMonths(date(a.harvestStart),-1))} → ${fmt(a.harvestStart)}</strong></div></div><div class="edag-datura">
<strong>🌿 Datura · ${esc(a.daturaRisk||"—")}</strong>
<small>📅 Période de prospection Datura : ${esc(a.daturaPeriod||"juin → octobre")}</small>
</div><div class="edag-explanation"><strong>Pourquoi prospecter ?</strong><p>${esc(a.explanation)}</p></div></div></article>`}).join('')}</div><p class="edag-note">ℹ️ Les périodes sont indicatives et peuvent varier selon l'année, la météo, la culture, la parcelle et les pratiques de l'exploitation.</p></section>`;c.querySelectorAll('[data-card]').forEach(card=>{const row=card.querySelector('.edag-row'),details=card.querySelector('.edag-details');const toggle=()=>{const open=!details.hidden;details.hidden=open;card.classList.toggle('expanded',!open);row.setAttribute('aria-expanded',String(!open))};row.addEventListener('click',toggle);row.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle()}})})}
